...
void QuadPlane::tiltrotor_continuous_update(void)
{
    ...
    if (!in_vtol_mode() && (!hal.util->get_soft_armed() || !assisted_flight)) {
        ...
        if (!hal.util->get_soft_armed()) {
            ...
            float throttle = tilt.current_throttle;
            float rud_gain = float(plane.g2.rudd_dt_gain) / 100;
            float rudder = rud_gain;
            ...
            // right : moter #2, 3, 4
            // rest : moter # 1, 8
            left_mask = (mask & 112); // 0 1 1 1 0 0 0 0 -> 112
            right_mask = (mask & 14); // 0 0 0 0 1 1 1 0 -> 14
            rest_mask = (mask & 129); // 1 0 0 0 0 0 0 1 -> 129
            motors->output_motor_mask_suyu(throttle, throttle_left, throttle_right, rest_mask, left_mask, right_mask);
            tilt.motors_active = true;
            }
        }
        return;
