...
void AP_MotorsMulticopter::output_motor_mask_suyu(float thrust, float thrust_left, float thrust_right, uint8_t mask_rest, uint8_t mask_left, uint8_t mask_right)
{
    for (uint8_t i=0; i<AP_MOTORS_MAX_NUM_MOTORS; i++) {
        if (motor_enabled[i]) {
            int16_t motor_out;
            if (mask_rest & (1U<<i)) {
                motor_out = calc_thrust_to_pwm(thrust);
            } else if (mask_left & (1U<<i)) {
                motor_out = calc_thrust_to_pwm(thrust_left);
            } else if (mask_right & (1U<<i)) {
